...
func main() {
	...
	go func() {
		...
		r := mux.NewRouter()
		//r.HandleFunc("/events/unauthenticated.VES_O_RAN_SC_HELLO_WORLD_PM_STREAMING_OUTPUT/myG/C1", sendDmaapMessages).Methods(http.MethodGet)
...
func sendDmaapMessages() {
	...
	for range time.Tick(10 * time.Second) {
		...
		//fmt.Printf("Sent message to consumer! Got response %v\n", r.Status)
